...
const ProductModal: React.FC<ProductModalProps> = ({ product, allProducts, suppliers, onClose, onSave }) => {
    ...
    useEffect(() => {
        setEditableProduct(isCreating ? {
            ...
            hasExpiryDate: false, alertQuantity: 0, barcode: ''
        } : product);
    }, [product, isCreating]);
    ...
    const handleSave = () => {
        ...
        let finalProduct = { ...editableProduct };
        
        // Auto-generate SKU if missing but supplier exists (existing logic)
        if (!finalProduct.sku && finalProduct.supplierId) {
             const supplier = suppliers.find(s => s.id === finalProduct.supplierId);
             if (supplier?.code) {
                 finalProduct.sku = `${supplier.code}-${Date.now().toString().slice(-6)}`;
             }
        }
        if (!finalProduct.sku) {
             finalProduct.sku = `GEN-${Date.now().toString().slice(-6)}`;
        }

        // Auto-generate Barcode if missing
        if (!finalProduct.barcode) {
            finalProduct.barcode = Math.floor(100000000000 + Math.random() * 900000000000).toString();
        }
                                ...
                                <div className="form-section-body" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                                    ...
                                    <FormField label="صورة المنتج">
                                        <div className="file-upload-area" style={{padding: '1rem'}}>
                                            ...
                                            <span>أفلت الصور هنا أو <span style={{color: 'var(--primary-color)', textDecoration: 'underline'}}>اختر من جهازك</span></span>
                                            <p style={{fontSize: '0.8rem', marginTop: '0.5rem', color: 'var(--text-secondary)'}}>مسموح برفع أكثر من صورة</p>
                                        </div>
                                    </FormField>
                                </div>
                            </div>
                                    ...
                                    <div className="form-grid-cols-2">
                                        <FormField label="الرقم التسلسلي SKU" required>
                                            <div style={{display: 'flex', alignItems: 'center', gap: '0.5rem'}}>
                                                <input type="text" value={editableProduct.sku || ''} onChange={e => handleChange('sku', e.target.value)} className="form-input" style={{flex: 1}} placeholder="يتم التوليد تلقائياً" />
                                        ...
                                        <FormField label="الباركود">
                                            <div style={{display: 'flex', alignItems: 'center', gap: '0.5rem'}}>
                                                <input type="text" value={editableProduct.barcode || ''} onChange={e => handleChange('barcode', e.target.value)} className="form-input" style={{flex: 1}} placeholder="يتم التوليد تلقائياً" />
                                            </div>
                                        </FormField>
