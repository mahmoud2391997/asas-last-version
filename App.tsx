...
const AppContent: React.FC = () => {
    ...
    const handleSaveShipment = (shipment: Shipment) => {
        const originalShipment = shipments.find(s => s.id === shipment.id);
        
        // If marking as Delivered, update inventory
        if (shipment.status === 'Delivered' && originalShipment?.status !== 'Delivered') {
            setInventory(prevInv => {
                const newInv = [...prevInv];
                
                shipment.items.forEach(item => {
                    // 1. Add to Destination
                    const destIndex = newInv.findIndex(i => i.branchId === shipment.destinationBranchId && i.productId === item.productId);
                    if (destIndex > -1) {
                        newInv[destIndex] = { ...newInv[destIndex], quantity: newInv[destIndex].quantity + item.quantity };
                    } else {
                        // Create new item if not exists
                        // Need to find minStock from product or default
                        newInv.push({
                    ...
                    // 2. Deduct from Source (if it's a branch)
                    if (shipment.sourceType === 'Branch') {
                        const sourceIndex = newInv.findIndex(i => i.branchId === shipment.sourceId && i.productId === item.productId);
                        if (sourceIndex > -1) {
                            newInv[sourceIndex] = { ...newInv[sourceIndex], quantity: newInv[sourceIndex].quantity - item.quantity };
                        }
                        // Note: We assume validation happened in the UI to prevent negative stock, 
                        // or we allow negative stock temporarily.
                    }
                });
                return newInv;
            });
            addToast('تم استلام الشحنة وتحديث المخزون.', 'success');
        }
        ...
        setShipments(prev => {
            ...
            return [...prev, { ...shipment, id: Date.now() }];
        });
        
        if (shipment.status !== 'Delivered') {
             addToast('تم حفظ الشحنة بنجاح.', 'success');
        }
    };
    ...
    const renderView = () => {
        ...
        if (activeView === 'Inventory/Warehouses') return <InventoryWarehouses inventory={inventory} branches={branches} products={products} />;
        ...
        // Supply Chain Module
        if (activeView.startsWith('SupplyChain/Supplies')) {
        ...
        if (activeView.startsWith('SupplyChain/Movements') || activeView === 'SupplyChain') {
            return <SupplyChainPage 
                mode="movements"
                shipments={shipments} 
                branches={branches} 
